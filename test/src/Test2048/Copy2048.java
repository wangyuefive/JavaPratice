...
public class Copy2048 extends JFrame{
	...
	private static final long serialVersionUID = 6435629927113034046L;
	...
	public Copy2048(){
		...
		textMaxScores = new JTextField("100000000");			//得分标签
		...
		tips = new JLabel("Tips：使用上、下、左、右键     作者：汪月");
		tips.setFont(font);
	...
	protected  void do_label_keyPressed(final KeyEvent e){
		...
		for(int i =0;i<4;i++)
			for(int j = 0; j<4 ; j++)
			{
				String str11 = texts[i][j].getText();
				setColor(i, j, str11);
			}
	...
	public void Create2(){
		...
		if(times > 0){
			while(!r){
				...
				int select = JOptionPane.showConfirmDialog(Copy2048.this, "你已经死了,重新来一发？！", "提示", 
